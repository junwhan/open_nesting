package edu.vt.rt.hyflow.benchmark.tm.LoanAgent;
	...
	extends 	AbstractDistinguishable
{
	...
	public static void borrow(String id, List accountNums, int amount){
		((LoanAccount)HyFlow.getLocator().open(id, "w")).borrow(accountNums, BRANCHING, true, amount);
	}
	...
	public static void sum(String id, List accountNums){
		((LoanAccount)HyFlow.getLocator().open(id, "r")).sum(accountNums, BRANCHING);
	}
